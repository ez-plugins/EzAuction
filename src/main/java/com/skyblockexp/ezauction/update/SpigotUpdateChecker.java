package com.skyblockexp.ezauction.update;
...
public final class SpigotUpdateChecker {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                ...
                if (!latestVersion.equalsIgnoreCase(currentVersion)) {
                    plugin.getLogger().info("A new EzAuction version is available: " + latestVersion
                            + " (current: " + currentVersion + "). Download: https://www.spigotmc.org/resources/"
                            + resourceId + "/");
                } else {
                    plugin.getLogger().info("EzAuction is up to date.");
                }
            } catch (Exception ex) {
                plugin.getLogger().warning("Failed to check for EzAuction updates: " + ex.getMessage());
            }
        });
    }
    ...
    private String fetchLatestVersion() throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "EzAuction Update Checker");
