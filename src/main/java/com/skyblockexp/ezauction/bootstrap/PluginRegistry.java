...
public class PluginRegistry {
    ...
    public void load() {
        ...
        if (!hologramSupportAvailable && configuration != null && configuration.hologramConfiguration().enabled()) {
    ...
    // Helper to set non-final fields (workaround for staged initialization pattern)
    ...
    public void enableAll() {
        // Register order menu listener (used in both modes)
        ...
        if (OrdersOnlyConfig.isOrdersOnlyMode()) {
            // Orders-only mode: only /orders, /order and order menu are registered above
            // Do not register auction, liveauction, hologram, or other auction features
            return;
        }
        
        // Normal mode: register all auction features
     ...
     * Note: This only reloads configuration data, not services or GUIs which remain with their original config.
     * For full reload, a server restart is recommended.
     */
    public void reloadConfiguration() {
        // Reload main plugin configuration
        plugin.reloadConfig();
        
        // Reload orders-only config
        OrdersOnlyConfig.load(plugin);
        
        // Reload auction configuration
        ConfigurationLoaderComponent configLoader = new ConfigurationLoaderComponent();
        AuctionConfiguration newConfig = configLoader.load(plugin);
        
        // Update configuration reference using reflection (same pattern as initialization)
        setField("configuration", newConfig);
        
        plugin.getLogger().info("EzAuction configuration files reloaded from disk. Note: Some settings require a server restart to take full effect.");
    }
